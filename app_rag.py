...
def get_thread_title(thread_id, max_len=40):
    ...
    title = " ".join(str(text).strip().splitlines())
...
# Update the cached title for a thread when new content is submitted
def update_thread_title(thread_id, text, max_len=40):
    if not text:
        return
...
if submission:
    add_thread(st.session_state["thread_id"])
    update_thread_title(st.session_state["thread_id"], submission.text)
...
if user_input:
    ...
    # Make sure the current thread is tracked before the next rerun
    add_thread(st.session_state["thread_id"])
